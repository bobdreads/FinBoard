...
def home(request):
    ...
    # --- CÁLCULO DO RESULTADO TOTAL CONVERTIDO ---
    total_pl_converted = 0
    for op in closed_operations:
        currency = op.account.currency
        conversion_date = op.end_date
        converted_result = convert_to_brl(
            op.net_financial_result, currency, conversion_date)
        total_pl_converted += converted_result
